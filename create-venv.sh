...
mkdir -p "$THIS_DIR/.venv/lib"
cp -f "$LIBDIR/libpython${LDV}.dylib" "$THIS_DIR/.venv/lib/libpython${LDV}.dylib"

# 4) Sanity check that the venv’s python runs:
otool -L "$THIS_DIR/.venv/bin/python3" | sed 's/^/  /'
.venv/bin/python3 -V

# 5) Use uv in full copy mode to install from your pyproject/uv.lock:
UV_LINK_MODE=copy uv sync --python "$THIS_DIR/.venv/bin/python3.11"

# 6) Prove there are no links in the venv:
find "$THIS_DIR/.venv" -type l -print