...
class MidiToOscForwarder:
    ...
    def _handle_midi_message(
        ...
        def _send() -> None:
            ...
            self._osc_client.send_message(self._osc_address, payload)

            for client in self._fanout_clients.get(controller, []):
                if client is self._osc_client:
                    continue
                client.send_message(self._osc_address, payload)
...
class MidiOscBridge:
    ...
    def __init__(
        ...
        *,
        ...
        self.osc_target_host = osc_target_host
        self.osc_target_port = osc_target_port
        self.osc_listen_host = osc_listen_host
        self.osc_listen_port = osc_listen_port
        self.osc_address = osc_address
        self.encoder_fanout = dict(encoder_fanout or {})

        self.midi_to_osc = MidiToOscForwarder(
            osc_host=self.osc_target_host,
            osc_port=self.osc_target_port,
            osc_address=self.osc_address,
            midi_in=midi_in,
            fanout=self.encoder_fanout,
        )
        self.osc_to_midi = OscToMidiForwarder(
            listen_host=self.osc_listen_host,
            listen_port=self.osc_listen_port,
            osc_address=self.osc_address,
            midi_out=midi_out,
        )
